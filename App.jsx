...
// OLD fetchViaAPI replaced — kept signature identical so nothing else needs to change

Rules:
- category must be one of: fruit_veg, meat, dairy, deli, dry, freezer
- ALL amounts must be metric (g, kg, ml, L). Countable items (eggs, cloves, cans) stay as numbers.
- Return 6-16 ingredients. Do NOT include method or steps.
- If you cannot determine any recipe from the URL, return {"error": "Cannot determine recipe from this link"}`;
...
Rules:
- Use ONLY the ingredients the user provided — do not invent extras.
- Convert all amounts to metric (g, kg, ml, L).
- Infer cuisine from title and ingredients.
- Use the method steps to accurately calculate prep time, cook time, and skill level.
- Do NOT return the method in the JSON — just use it for time/skill inference.`;
