...
export default function App() {
  ...
  function showBanner(msg) {
    ...
    setBannerTimer(setTimeout(() => { setBanner(""); setBannerTimer(null); }, 3000));
  }
  ...
  function rebuildShoppingList(wk, lib) {
    ...
    // Use the ref so async callers always see the latest list, not a stale closure snapshot.
    const prev = shoppingListRef.current;
    const items = DAYS
    ...
    const auto = buildShoppingList(items).map((item) => {
      const existing = prev.find((i) => i.item.toLowerCase() === item.item.toLowerCase());
      return { ...item, checked: existing?.checked || false };
    });
    // Preserve manually-added items
    const manual = prev.filter((i) => i.manual);
              ...
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 20 }}>
                ...
                {checkedCount > 0 && (
                  <button style={{ ...btnStyle, background: "var(--bg3)", color: "var(--text2)", padding: "7px 14px", fontSize: 12 }}
                    onClick={async () => {
                      const updatedList = shoppingList.filter((i) => !i.checked);
                      const updatedCleaning = cleaning.filter((i) => !i.checked);
                      const updatedPharmacy = pharmacy.filter((i) => !i.checked);
                      setShoppingList(updatedList); setCleaning(updatedCleaning); setPharmacy(updatedPharmacy);
                      await saveList(updatedList, { immediate: true });
                      await saveCleaning(updatedCleaning);
